...
def print_fhir_summary(library: dict):
    ...
    if extensions:
        ...
        for ext in extensions[:3]:  # Show first 3
            ...
            value_key = [k for k in ext.keys() if k.startswith('value')][0] if any(k.startswith('value') for k in ext.keys()) else 'url'
    ...
    */
        ...
        if sql_files:
            ...
            for sql_file in sql_files:
                ...
                try:
                    ...
                    for i, (key, value) in enumerate(list(file_annotations.items())[:3]):
                        print(f"  @{key}: {value}")
                    ...
                    output_path = builder.export_library_to_file(file_library, output_filename)
    ...
    if examples_dir.exists() and list(examples_dir.glob("*.sql")):
        ...
        for i, lib in enumerate(libraries, 1):
            ...
            error_ext = next((ext for ext in lib.get('extension', []) 
                            if 'error' in ext.get('url', '')), None)
            
            if error_ext:
                print(f"  {i}. ERROR - {lib.get('id', 'unknown')}: {error_ext.get('valueString', 'Unknown error')}")
            else:
